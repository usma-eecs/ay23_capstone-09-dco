...
#        {
        ...
        # print rec;
#        if (rec?$host && rec?$user_agent && /oneocsp/ in rec$host && /Microsoft-CryptoAPI\// in rec$user_agent)
#                {
...
event connection_SYN_packet(c: connection, pkt: SYN_packet) &priority=5
        {
                print pkt;
        }
~          
